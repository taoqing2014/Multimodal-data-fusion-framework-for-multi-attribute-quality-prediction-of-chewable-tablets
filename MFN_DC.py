...
class HSI_NIR_Regression_MFN_DC(nn.Module):
    """完整的光谱-空间特征融合回归网络"""

    def __init__(self, feature_dim=32, output_dim=1):
        super(HSI_NIR_Regression_MFN_DC, self).__init__()
...
def train_models():
    ...
    for ylabel_idx in [4]:#[0,1,2,3,4]:
        for feature_dim in [32, 48, 64, 128, 256]:
            ...
            # 创建模型实例
            model = HSI_NIR_Regression_MFN_DC(feature_dim=feature_dim ).to(device)
            ...
            save_path = f'MFN_DC/FeatureDIM_{feature_dim}_model_{ylabel_idx}.pth'
            # 保存模型
...
def test_best_models_forSavePrediction():
    ...
    for label, val in results.items():
        #ypred = val['ypred']
        save_prediction_path = f'MFN_DC/ypred_{label}.mat'
